...
void handle_small_stack(node **src, node **dst, int size, int *pushed)
{
    if (!src || !(*src))
        return;
    printf("Handle small %d\n", (*src)->data);
    if ((*src)->next)
        printf("Handle small second %d\n", (*src)->next->data);
    if (size == 2)
    {
...
void b_partition(node **src, node **dst, int partition_size, int *pushed)
{
    ...
    while (*src && i < partition_size && (*pushed) < count_bigger)
    {
        ...
        if ((*src)->data > median && !has_bigger((*src)->data, *src))
        {
            ...
            if (count_bigger - *pushed == 2 && partition_size - (*pushed) - rotations == 2)
            {
                handle_small_stack(src, dst, 2, pushed);
            }
            else if ((*src)->data > median && !has_bigger((*src)->data, *src))
            {
                push_to_stack(src, dst);
                ft_putstr_fd("pa\n", 1);
                (*pushed)++;
